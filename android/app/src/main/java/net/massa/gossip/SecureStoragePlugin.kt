...
class SecureStoragePlugin : Plugin() {
    ...
    fun execSql(call: PluginCall) {
        ...
        val convertMs = (System.nanoTime() - tConvert) / 1_000_000
        ...
        executor.execute {
            ...
            try {
                ...
                val execMs = (System.nanoTime() - tExec) / 1_000_000

                val tSerialize = System.nanoTime()
                val result = JSObject()
                val cols = JSArray()
                ...
                val serializeMs = (System.nanoTime() - tSerialize) / 1_000_000

                val sqlType = sql.trimStart().take(6).uppercase()
                val totalMs = convertMs + queueMs + execMs + serializeMs
                if (totalMs > 5) {
                    android.util.Log.d("NativePerf", "$sqlType(${totalMs}ms) queue=$queueMs exec=$execMs ser=$serializeMs conv=$convertMs: ${sql.take(80)}")
                }
                call.resolve(result)
            } catch (e: SecureStorageException) { call.reject(e.message, e) }
        }
