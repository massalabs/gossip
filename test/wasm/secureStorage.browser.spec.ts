...
const SECURE_STORAGE_TEST_TIMEOUT_MS = 60_000;
...
describe('secure-storage wasm browser integration', () => {
  ...
  it(
    'keeps SQLite handles session-bound across implicit session switches',
    async () => {
    ...
    SECURE_STORAGE_TEST_TIMEOUT_MS
  );

  it(
    'rejects generic namespace APIs for the SQLite namespace',
    () => {
      allocateSession(0, PASSWORD_A);
  ...
  it(
    'opens SQLite, executes SQL, and flushes encrypted data to IndexedDB',
    async () => {
      allocateSession(0, PASSWORD_A);
      openDatabase();
