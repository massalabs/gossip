...
export type StorageConfig =
  ...
  | {
      type: 'secureStorage';
      domain: string;
      /** Encrypted storage backend. Always IDB (IndexedDB). */
      backend?: 'idb';
      wasmUrl?: string;
    };
...
export class DatabaseConnection {
  ...
  private async execRawDirect(
    ...
    if (this.state.useNativePlugin) {
      ...
      const sqlType = sql.trimStart().split(/\s/)[0].toUpperCase();
      console.log(`[NativePerf] ${sqlType}(${dt}ms): ${sql.slice(0, 80)}`);
    ...
    if (this.state.useWorker) {
      ...
      const dt = (performance.now() - t0) | 0;
      const sqlType = sql.trimStart().split(/\s/)[0].toUpperCase();
      console.log(`[WorkerPerf] ${sqlType}(${dt}ms): ${sql.slice(0, 80)}`);
      this.state.lastInsertRowIdCache = result.lastInsertRowId;
      return result.rows;
    }
