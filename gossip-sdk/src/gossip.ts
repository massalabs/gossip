...
import {
  ...
  getDiscussionsByOwnerAndStatus,
} from './queries';
import {
  getContacts,
  getContact,
  ...
  private createServiceAPIWrappers(session: SessionModule): void {
    ...
    this._discussionsAPI = {
      ...
      get: async (ownerUserId, contactUserId) => {
        ...
        return row
          ? rowToDiscussion(row as Record<string, unknown>)
          : undefined;
      },
    };
    ...
    this.pollingManager.start(
      config,
      {
        ...
        getActiveDiscussions: async () => {
          const rows = await getDiscussionsByOwnerAndStatus(
            session.userIdEncoded,
            DiscussionStatus.ACTIVE
          );
          return rows.map(r => rowToDiscussion(r as Record<string, unknown>));
        },
      },
      this.eventEmitter
    );
