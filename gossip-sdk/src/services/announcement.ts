...
export class AnnouncementService {
  ...
  async processOutgoingAnnouncements(discussions: Discussion[]): Promise<void> {
    ...
    for (const discussion of discussions) {
      ...
      // sendAnnouncement is deserialized by rowToDiscussion() before reaching here
  ...
  private async _handleReceivedDiscussion(
    ...
    if (existing) {
      ...
      if (
        ...
        this.eventEmitter.emit(
          ...
          rowToDiscussion(newDiscussion as Record<string, unknown>),
          contactRow
        );
      }
    ...
    if (discussion && contactRow) {
      this.eventEmitter.emit(
        SdkEventType.SESSION_REQUESTED,
        rowToDiscussion(discussion as Record<string, unknown>),
        contactRow
      );
    }
