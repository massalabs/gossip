  ...
  useEffect(() => {
    const loadProfile = async () => {
      try {
        ...
        if (existingProfile) {
          // Only promote to initialized on FIRST load (app startup).
          // During onboarding, isInitialized starts false and stays false
          // until SecureStorageSetup/Onboarding explicitly sets it.
          // Re-checking here would short-circuit the secure storage setup flow.
          if (!alreadyInitialized && !useAccountStore.getState().userProfile) {
            // Profile in DB but not in memory → returning user, show login
            useAppStore.getState().setIsInitialized(true);
          }
        } else if (!alreadyInitialized) {
          useAppStore.getState().setIsInitialized(false);
