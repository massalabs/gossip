...
const MessageBubble: React.FC<MessageBubbleProps> = ({
  ...
  return (
    <div
      ref={bubbleRef}
      className={`relative max-w-[80%] sm:max-w-[70%] md:max-w-[65%] lg:max-w-[60%] ${hasMultipleReactions ? 'min-w-[8rem]' : ''} px-3.5 py-3 font-normal text-[15px] leading-tight ${isTextSelectable ? 'select-text' : 'select-none'} ${borderRadiusClass} ${
        isOutgoing
          ? 'ml-auto mr-3 bg-accent text-accent-foreground'
          : `${hasContact ? '' : 'ml-3'} mr-auto bg-surface-secondary text-card-foreground`
