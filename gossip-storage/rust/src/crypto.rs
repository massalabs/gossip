...
pub fn unlock_with_keys(
    ...
    let mut _scanned = 0u32; // Counter to prevent loop optimization

    // ALWAYS iterate all 46 slots - no early exit
    for position in 0..SLOTS_PER_SESSION {
        _scanned += 1;

        let (slot_index, key) = session_keys.get_slot_info(position)?;
    ...
    // Verify we scanned all slots
    debug_assert_eq!(_scanned, SLOTS_PER_SESSION as u32);
...
pub fn unlock_session(
    ...
    unlock_with_keys(&session_keys, get_slot)
}
