...
export class PlausibleDeniableVFS extends VFS.Base {
  ...
  private stats = { readCount: 0, writeCount: 0 };
  ...
  reset(): void {
    ...
    this.stats = { readCount: 0, writeCount: 0 };
  }

  /**
   * Get I/O statistics
   */
  getStats(): { readCount: number; writeCount: number } {
    return { ...this.stats };
  }
  ...
  xSync(_fileId: number, _flags: number): number {
    // Don't flush to storage on every sync - this causes partial blocks
    // to be written (e.g., 512 bytes instead of 4096).
    // Data is flushed when:
    // 1. Session is locked (explicit flush)
    // 2. Write buffer exceeds minimum block size (automatic in Rust)
    // SQLite's crash recovery still works because wa-sqlite handles journaling.
    return VFS.SQLITE_OK;
  }
