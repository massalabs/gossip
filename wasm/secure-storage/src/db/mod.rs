...
pub fn open(vfs_name: &str) -> Result<(), SecureStorageError> {
    ...
    // NORMAL: SQLite calls xSync at COMMIT, which triggers the
    // pending-writes flush (coalescing N page writes into a single
    // batch of PQ operations). Persistence to OPFS is still handled
    // by the periodic flush timer and explicit flushes on lock.
