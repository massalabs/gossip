...
/// Configure rayon thread pool with PQ-appropriate stack size (native only).
...
/// Execute PQ jobs in parallel using rayon (native).
#[cfg(not(target_arch = "wasm32"))]
fn execute_pq_jobs(jobs: Vec<PqJob>) -> Result<HashMap<(u64, u8), Vec<u8>>> {
    ...
    let results: Vec<(u64, u8, Vec<u8>)> = jobs.into_par_iter().map(run_pq_job).collect();
    let mut map = HashMap::with_capacity(results.len());
    for (bi, si, ct) in results {
        map.insert((bi, si), ct);
    }
    Ok(map)
}

/// Sequential fallback for WASM (shared memory blocked by sqlite-wasm-rs precompiled).
