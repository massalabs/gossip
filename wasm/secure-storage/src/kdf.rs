...
/// Derive the per-block AEAD key and block scope for a given session, namespace
/// and block index.
///
/// Each `(session, namespace, block)` triple produces a unique AEAD key —
/// the namespace byte is mixed into both the per-block KDF salt (via
/// `block_kdf_salt`) and the HKDF Extract input items, so even with the
/// same root key the keys are cryptographically isolated across namespaces.
///
...
pub fn derive_block_aead_key(
    ...
    block_kdf_salt(&mut buf, domain, version, index, namespace, block);
    let expander = {
        ...
        extract.input_item(&[namespace]);
        extract.input_item(&block.to_be_bytes());
        extract.finalize()
    };

    block_aead_key_label(&mut buf, domain, version, index, namespace, block);
    let mut key = Zeroizing::new([0u8; crypto_aead::KEY_SIZE]);
    expander.expand(buf.as_bytes(), key.as_mut());

    block_scope(&mut buf, domain, version, index, namespace, block);
    (key, buf)
}
...
mod tests {
    ...
    fn derive(
        ...
        let (key, _scope) =
            derive_block_aead_key(domain, version, idx(session), namespace, root, block);
        *key
    }
