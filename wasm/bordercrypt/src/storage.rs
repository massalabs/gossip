...
/// Block-level storage for session blockstreamstream files.
///
/// Each session has a contiguous sequence of fixed-size blockstreams.
pub trait BlockStorage {
    /// Read a block at the given index from a session's blockstreamstream.
    fn read_block(&self, session: SessionIndex, block: u64) -> Result<Box<[u8; BLOCK_SIZE]>>;

    /// Write a block at the given index to a session's blockstreamstream.
    fn write_block(
        &mut self,
    ...
    /// Append a new block at the end of a session's blockstreamstream.
    fn append_block(&mut self, session: SessionIndex, data: &[u8; BLOCK_SIZE]) -> Result<()>;

    /// Number of blockstreams currently in a session's blockstreamstream.
