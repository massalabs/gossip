...
use crate::error::BordercryptError;
...
thread_local! {
    static STATE: RefCell<Option<BordercryptState>> = const { RefCell::new(None) };
}
...
pub fn init_bordercrypt(domain: &str) {
    STATE.with(|s| {
        // Assignment drops old state first — UnlockedSession's ZeroizeOnDrop
        // ensures any prior session keys are zeroized before replacement.
        *s.borrow_mut() = Some(BordercryptState {
...
pub fn unlock(password: &[u8]) -> Result<bool, JsValue> {
    STATE.with(|s| {
        ...
        match unlock_session(&state.storage, &state.domain, password) {
            ...
            Err(BordercryptError::InvalidPassword) => Ok(false),
            Err(e) => Err(map_err(e)),
        }
    })
}
